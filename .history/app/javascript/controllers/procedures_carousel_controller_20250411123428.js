...
export default class extends Controller {
  ...
  setupContainerWidth() {
    ...
    const totalWidth = (itemWidth * this.itemTargets.length) + (gap * (this.itemTargets.length - 1))
  ...
  handleTouchMove(event) {
    ...
    const itemWidth = this.itemTargets[0].offsetWidth
    const gap = 10
    const translateX = -this.currentPageValue * (itemWidth * 2 + gap) + diff
  ...
  isLastItemFullyVisible() {
    ...
    const maxTranslateX = -(this.itemTargets.length - 2) * (this.itemTargets[0].offsetWidth + 10)
  ...
  resetPosition() {
    ...
    const itemWidth = this.itemTargets[0].offsetWidth
    const gap = 10
    const translateX = -this.currentPageValue * (itemWidth * 2 + gap)
    this.containerTarget.style.transform = `translateX(${translateX}px)`
  }
