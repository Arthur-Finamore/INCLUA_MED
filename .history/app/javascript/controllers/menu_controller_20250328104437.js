...
// Adiciona o event listener de 'click' À IMAGEM
fullscreenButton.addEventListener("click", () => {
  if (document.fullscreenElement) {
    document.exitFullscreen();
    updateFullscreenText(); // Atualiza o texto
  } else {
...
// Listener para mudanças no fullscreen via API
document.addEventListener('fullscreenchange', updateFullscreenText);

// Listener para detectar F11 (fullscreen do navegador)
document.addEventListener('keydown', (e) => {
  if (e.key === 'F11') {
    // Usamos setTimeout porque o evento keydown ocorre antes do fullscreen realmente mudar
    setTimeout(updateFullscreenText, 100);
  }
});

// Função para atualizar o texto conforme o estado do fullscreen
function updateFullscreenText() {
  ...
  if (document.fullscreenElement || window.innerHeight === screen.height) {
    fullscreenText.textContent = 'MINIMIZAR';
  } else {
    fullscreenText.textContent = 'MAXIMIZAR';
  }
}
