...
function updateFullscreenText() {
  ...
  // Verifica se está em fullscreen (via API ou F11)
...
fullscreenButton.addEventListener("click", () => {
  ...
  // Não chamamos updateFullscreenText() aqui porque o fullscreenchange já cuida disso
});

// Detecta mudanças via API Fullscreen (click no botão ou ESC)
document.addEventListener('fullscreenchange', updateFullscreenText);

// Detecta F11 (entrada e saída)
document.addEventListener('keydown', (e) => {
  if (e.key === 'F11') {
    // Usamos setTimeout porque o F11 só aplica após o evento
    setTimeout(updateFullscreenText, 100);
  }
});

// Detecta redimensionamento manual (útil para quando sai do F11)
