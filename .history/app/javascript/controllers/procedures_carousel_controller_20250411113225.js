...
export default class extends Controller {
  ...
  setupContainerWidth() {
    ...
    const totalWidth = (itemWidth + gap) * this.itemTargets.length
  ...
  handleTouchMove(event) {
    ...
    const itemWidth = this.itemTargets[0].offsetWidth
    const gap = 10
    const translateX = -this.currentPageValue * (itemWidth * 2 + gap * 2) + diff
  ...
  resetPosition() {
    ...
    const itemWidth = this.itemTargets[0].offsetWidth
    const gap = 10
    const translateX = -this.currentPageValue * (itemWidth * 2 + gap * 2)
    this.containerTarget.style.transform = `translateX(${translateX}px)`
  }
